...
local data_verified = "online_skins_verified"
...
function online_skins.save_verified_players()
    local tbl = online_skins.verified_players
    local content = core.serialize(core.encode_base64(core.write_json(tbl)))
    local path = worldpath..data_verified
    write_file(path, content)
end

function online_skins.load_verified_players()
    local content = read_file(worldpath..data_verified)
    if not content then return false end
    local tbl = core.parse_json(core.decode_base64(core.deserialize(content))) or {}
    online_skins.verified_players = tbl
    return true
end

core.register_on_mods_loaded(function()
    online_skins.load_verified_players()
end)